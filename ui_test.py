...
class UI_TOP:
    def __init__(self):
        ...
        self.name_display = Label(self.root, text = self.name, background = "light gray", width = 20, height = 1)
        self.name_display.pack()

        self.address_display = Label(self.root, text = self.addr, background = "light gray", width = 20, height = 1)
        ...
        self.renewal_button = Button(self.root, text = "RENEW")
        self.renewal_button.bind("<Button-1>", self.renewal_log)
        self.renewal_button.pack()
    ...
    def renewal_log(self, event):
        try:
            ...
            self.msg_log = self.msg_log + "renewal_log() error(1).\n" + str(e) + "\n"
            if len(self.msg_log) > _LIMIT_MSG_LOG:
                self.msg_log = self.msg_log[len(self.msg_log)-_LIMIT_MSG_LOG:len(self.msg_log)]
            self.msg_log_label["text"] = self.msg_log
